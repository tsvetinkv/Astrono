...
function fetchTranslationsForPopup(func) {
    let currentLanguage = window.currentLanguage; // 'en' for English or 'bg' for Bulgarian
    const fetchUrl = `./popupTranslations.json`; // Replace with your JSON file's URL
    ...
    fetch(fetchUrl, {
        ...
        .then(data => {
            func(data.data); // Pass the `data` array to the callback
        })
...
elevationLearnMore.addEventListener("click", () => {
    ...
    function translateTxt(translations) {
        translations.forEach(t => {
            if (t.id === 206) {
                typeMoon.name = t[`text_${window.currentLanguage}`]; // Use dynamic key based on language
            }
            if (t.id === 228) {
                typeMoon.figcaption = t[`text_${window.currentLanguage}`];
            }
            if (t.id === 234) {
                typeMoon.description = t[`text_${window.currentLanguage}`];
            }
        });
...
gravityLearnMore.addEventListener("click", () => {
    ...
    function translateTxt(translations) {
        translations.forEach(t => {
            if (t.id === 238) {
                typeMoon.name = t[`text_${window.currentLanguage}`];
            }
            if (t.id === 237) {
                typeMoon.figcaption = t[`text_${window.currentLanguage}`];
            }
